...
class Positioning(object):
    ...
    class Edge(object):
        ...
        def __init__(self, frm, to):
            self.frm = frm
            self.to = to
            #p0, p1, p2, dec_y, x1 or x2 depending edge simple or complex
    ...
    def pass_on_block(self, e, loop=False):
        ...
        conflict_block = [x for x in self.original_order
                          if (n_frm < self.original_order[x] < n_to
        ...
        for b in self.original_order:
            if b is not (e.frm or e.to):
                for tmp_e in b.edges_to:
                    if (
                        (self.original_order[e.frm] <= self.original_order[tmp_e.frm] < self.original_order[e.to])
                        or (e.frm <= self.original_order[tmp_e.to] < e.to)
        ...
        if (self.test_elt_not_in_list_range(int(ex_f), unused_range)
            and self.test_elt_not_in_list_range(int(ex_f)+1,
                                                unused_range)):
            return ex_f
    ...
    def check_edge_to(self, e, pos, config):
    ...
    def auto_fix_pos_edge(self, cross_edge=False):
        for b in self.original_order:
            if cross_edge:
                b.edges_frm.sort(key=lambda x: (x.path[4]))
            else:
                b.edges_frm.sort(key=lambda x: (x.frm.x))
            config = self.get_config_edges(b, cross_edge)
            posfrm = 0
        ...
        for b in self.original_order:
            if cross_edge:
                b.edges_to.sort(key=lambda x: (x.path[5]))
            else:
    ...
    def auto_set_y(self, bottom=False):
        ...
        for i in range(max_order + 1):
            ...
            for j in range(1, len(current)):
                print("j2: %d"%j)
                if bottom:
                    max_block_left = max(y0_left, current[j][0].y - self.edge_dist)
                else:
                    max_block_left = max(y0_left, current[j][0].y + current[j][0].h + self.edge_dist)
